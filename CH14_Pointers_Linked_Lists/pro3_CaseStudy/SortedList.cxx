//******************************************************************
// IMPLEMENTATION FILE (SortedGeneral.cxx)
...
void SortedList::Delete(ItemType item)
{
  ...
  while ((currPtr != NULL) && (currPtr->component.ComparedTo(item) != SAME) && (currPtr->component.ComparedTo(item) == BEFORE))
  {
    prevPtr = currPtr;
    currPtr = currPtr->link;
  }
  if (currPtr != NULL && currPtr->component.ComparedTo(item) == SAME)
  { // item is found
    if (currPtr == dataPtr)
