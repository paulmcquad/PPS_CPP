...
//******************************************************************
// IMPLEMENTATION FILE (Sortedlist.cxx)
// This file gives the implementation of a SortedList abstract data
...
// If item is found, found is true and position is it's index;
// else found is false and position is undefined
{
  ...
  while (last >= first && !found)
  {
    middle = (first + last) / 2;
    if (item < data[middle])
      // item is not in data[middle..last]
      last = middle - 1;
    else if (item > data[middle])
      // item is not in data[first..middle]
      first = middle + 1;
    else
      // item == data[middle]
      found = true;
